...
type Payout = { id: string; amount: number | string; status: string; dueDate: string; reference?: string; currency?: string };

export default function Payouts() {
  const [rows, setRows] = useState<Payout[]>([]); const [error,setError]=useState('');
  useEffect(() => { api.request<{ payouts: Payout[]; legacyPayouts: Payout[] }>('/settlements').then((result) => setRows([...result.payouts, ...result.legacyPayouts])).catch((e)=>setError(e.message)); }, []);
  const totals = useMemo(() => ({ scheduled: rows.filter(r=>['SCHEDULED','READY_TO_RELEASE'].includes(r.status)).reduce((a,b)=>a+Number(b.amount),0), transit: rows.filter(r=>['IN_TRANSIT','RELEASE_FAILED','RECONCILIATION_REQUIRED'].includes(r.status)).reduce((a,b)=>a+Number(b.amount),0), paid: rows.filter(r=>['PAID','RELEASED','RECONCILED'].includes(r.status)).reduce((a,b)=>a+Number(b.amount),0) }), [rows]);
