...
export default function Bookings() {
  ...
  async function voucher(id: string) { try { const result = await api.request<{ voucherCode: string; fileId: string }>(`/bookings/${id}/voucher`); const blob = await api.blob(`/files/${result.fileId}/content`); setVoucherUrl((old) => { if (old) URL.revokeObjectURL(old); return URL.createObjectURL(blob); }); } catch (e) { setError((e as Error).message); } }
  if (loading) return <Loading />;
  const visible = rows.filter((r) => filter === 'ALL' || (filter === 'NEEDS_ACTION' ? r.status === 'PENDING' : r.status === filter));

  return <>{error && <ErrorBox error={error} />}<div className="toolbar"><div className="chips">{[['ALL','All'],['NEEDS_ACTION','Needs Action'],['CONFIRMED','Confirmed'],['CANCELLED','Cancelled']].map(([v,l]) => <button key={v} onClick={() => setFilter(v)} className={`chip ${filter === v ? 'active' : ''}`}>{l}</button>)}</div></div><Panel><div className="table-wrap"><table><thead><tr><th>Booking ID</th><th>Product</th><th>Source</th><th>Date</th><th>Pax</th><th>Amount</th><th>Status</th><th>Action</th></tr></thead><tbody>{visible.map((b) => <tr key={b.id}><td className="mono">{b.bookingCode}</td><td>{b.product.currentRevision?.productName || b.product.productCode}</td><td>{b.channel}</td><td>{shortDate(b.serviceDate)}</td><td>{b.pax}</td><td>{money(b.amount)}</td><td><Badge value={b.status} /></td><td><div className="row-actions">{b.status === 'PENDING' && <button className="btn accent small" onClick={() => action(b.id,'confirm')}>Confirm</button>}{['PENDING','CONFIRMED'].includes(b.status) && <button className="btn small" onClick={() => action(b.id,'cancel')}>Cancel</button>}{['CONFIRMED','COMPLETED'].includes(b.status) && <button className="btn small" onClick={() => voucher(b.id)}>Voucher</button>}{b.status === 'CANCELLED' && <button className="btn small">Details</button>}</div></td></tr>)}</tbody></table></div></Panel>{voucherUrl && <div className="modal-backdrop" onClick={() => { URL.revokeObjectURL(voucherUrl); setVoucherUrl(''); }}><div className="voucher-modal" onClick={(e) => e.stopPropagation()}><div className="modal-head"><h3>Booking Voucher</h3><button className="btn small" onClick={() => { URL.revokeObjectURL(voucherUrl); setVoucherUrl(''); }}>Close</button></div><iframe title="Booking voucher PDF" src={voucherUrl} /></div></div>}</>;
}
