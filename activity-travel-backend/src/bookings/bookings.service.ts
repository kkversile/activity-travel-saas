...
export class BookingsService {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        return await this.prisma.$transaction(async (tx) => {
          ...
          const schedule = await tx.activitySchedule.findFirst({ where: { id: dto.scheduleId, activityId: dto.activityId, isBookable: true, startsAt: { gt: new Date() } } });
          ...
          const booking = await tx.booking.create({
            data: {
              tenantId, activityId: activity.id, scheduleId: schedule.id,
              ...
              notes: dto.notes, voucherCode: dto.voucherCode, holdExpiresAt: new Date(Date.now() + 15 * 60 * 1000), passengers: { create: dto.passengers }
  ...
  async list(tenantId: string) {
    await this.releaseExpiredHolds(tenantId);
    return this.prisma.booking.findMany({ where: { tenantId }, include: { activity: true, schedule: true, passengers: true }, orderBy: { createdAt: "desc" }, take: 100 });
  }
  ...
  async confirm(tenantId: string, id: string) {
    ...
    const updated = await this.prisma.booking.update({ where: { id }, data: { status: BookingStatus.CONFIRMED, holdExpiresAt: null }, include: { activity: true, schedule: true, passengers: true } });
  ...
  async getById(tenantId: string, id: string) {
    const booking = await this.prisma.booking.findFirst({ where: { id, tenantId }, include: { activity: { include: { cancellationRules: true } }, schedule: true, passengers: true } });
    if (!booking) throw new NotFoundException("Booking not found");
    return booking;
  }

  listVouchers(tenantId: string) { return this.prisma.voucher.findMany({ where: { tenantId }, orderBy: { createdAt: "desc" } }); }
  ...
  listCustomers(tenantId: string) { return this.prisma.customer.findMany({ where: { tenantId }, include: { _count: { select: { bookings: true } } }, orderBy: { name: "asc" } }); }
