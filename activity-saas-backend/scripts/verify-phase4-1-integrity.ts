...
async function integrityCase() {
  ...
  const invalidResources = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM "ScheduleResourceRequirement" r LEFT JOIN "Resource" x ON x."id" = r."specificResourceId" WHERE r."specificResourceId" IS NOT NULL AND r."active" = true AND r."archivedAt" IS NULL AND (x."id" IS NULL OR x."tenantId" <> (SELECT p."tenantId" FROM "ScheduleTemplate" st JOIN "ProductVariant" v ON v."id" = st."variantId" JOIN "Product" p ON p."id" = v."productId" WHERE st."id" = r."scheduleTemplateId") OR x."type" <> r."resourceType" OR x."active" = false OR x."archivedAt" IS NOT NULL)`);
