...
async function integrityCase() {
  const negative = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM "public"."InventoryState" WHERE "totalCapacity" < 0 OR "blockedCapacity" < 0 OR "heldCapacity" < 0 OR "confirmedCapacity" < 0`);
  const violations = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM "public"."InventoryState" WHERE "totalCapacity" < "blockedCapacity" + "heldCapacity" + "confirmedCapacity"`);
  const duplicateScopes = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM (SELECT "scheduleTemplateId", "serviceDate", "slotTemplateId" FROM "public"."ScheduleException" WHERE "archivedAt" IS NULL GROUP BY 1,2,3 HAVING count(*) > 1) x`);
  const crossScheduleSlots = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM "public"."ScheduleException" e JOIN "public"."ScheduleSlotTemplate" s ON s."id" = e."slotTemplateId" WHERE e."slotTemplateId" IS NOT NULL AND e."scheduleTemplateId" <> s."scheduleTemplateId"`);
  const invalidResources = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM "public"."ScheduleResourceRequirement" r LEFT JOIN "public"."Resource" x ON x."id" = r."specificResourceId" WHERE r."specificResourceId" IS NOT NULL AND r."active" = true AND r."archivedAt" IS NULL AND (x."id" IS NULL OR x."tenantId" <> (SELECT p."tenantId" FROM "public"."ScheduleTemplate" st JOIN "public"."ProductVariant" v ON v."id" = st."variantId" JOIN "public"."Product" p ON p."id" = v."productId" WHERE st."id" = r."scheduleTemplateId") OR x."type" <> r."resourceType" OR x."active" = false OR x."archivedAt" IS NOT NULL)`);
  const duplicateAssignments = await prisma.$queryRawUnsafe<any[]>(`SELECT count(*)::int AS count FROM (SELECT "sessionId", "resourceId" FROM "public"."SessionResourceAllocation" WHERE "status" = 'ACTIVE' GROUP BY 1,2 HAVING count(*) > 1) x`);
  ...
  assert(Number(negative[0].count) === 0, 'no negative inventory counters'); assert(Number(violations[0].count) === 0, 'no capacity violations'); assert(Number(duplicateScopes[0].count) === 0, 'no duplicate active exception scopes'); assert(Number(crossScheduleSlots[0].count) === 0, 'no cross-schedule exception slots'); assert(Number(invalidResources[0].count) === 0, 'no invalid active specific resources'); assert(Number(duplicateAssignments[0].count) === 0, 'no duplicate active resource assignment identities'); assert(reviewRequired > 0, 'legacy schedules are review-gated'); assert(confirmed >= 0, 'capacity-unit confirmation state is queryable');
  console.log(JSON.stringify({ pass: true, sessions, plans, bookings, historicalFixtureBaseline: { expectedSessions: 110, expectedPlans: 40, expectedBookings: 5, drift: sessions !== 110 || plans !== 40 || bookings !== 5 }, reviewRequired, confirmed, negative: Number(negative[0].count), capacityViolations: Number(violations[0].count), duplicateExceptionScopes: Number(duplicateScopes[0].count), crossScheduleSlots: Number(crossScheduleSlots[0].count), invalidActiveSpecificResources: Number(invalidResources[0].count), duplicateActiveAssignments: Number(duplicateAssignments[0].count) }, null, 2));
}
