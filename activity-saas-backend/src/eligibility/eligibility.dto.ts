...
export class EligibilityRequestDto {
  ...
  @Type(() => EligibilityTravellerDto)
  travellers!: EligibilityTravellerDto[];
  @IsOptional() @Type(() => Number) @IsInt() @Min(1) units?: number;
  @ApiPropertyOptional({
    description:
      "Accepted for privileged diagnostics and deterministic tests only.",
  })
  @IsOptional()
  @IsDateString()
  now?: string;
...
export class MarketplaceSearchDto {
  ...
  @ApiProperty({ description: "Stable UUID v4 for one deliberate search attempt; reuse it for transport retries." })
  @IsUUID("4") searchAttemptId!: string;
  ...
  @IsOptional()
  @IsIn(["PRIVATE", "SHARED", "Private", "Shared"])
