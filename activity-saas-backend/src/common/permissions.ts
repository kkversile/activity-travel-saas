...
export const PERMISSIONS = [
  ...
  'demand.view.admin', 'demand.manage', 'demand.policy.manage', 'demand.target.manage', 'demand.view.vendor', 'demand.respond.vendor',
  'distribution.view', 'distribution.channel.manage', 'distribution.contract.manage', 'distribution.mapping.manage', 'distribution.inventory-rule.manage', 'distribution.credential.manage', 'distribution.event.view', 'distribution.inspect',
] as const;
...
export function permissionsFor(user: Pick<AuthUser, 'role' | 'organizationRole'>): Set<Permission> {
  ...
  if (user.role === UserRole.SUB_ADMIN) return new Set(['vendor.profile.view', 'document.view', 'document.review', 'product.view', 'product.publish', 'booking.view', 'booking.view.admin', 'booking.manual.decide', 'booking.expiry.run', 'payout.view', 'settlement.view', 'reconciliation.view', 'audit.view', 'commercial.internal.view', 'commercial.internal.edit', 'commercial.agent-groups.manage', 'commercial.quote.internal', 'eligibility.inspect', 'agent.governance.view', 'agent.governance.edit', 'marketplace.channel.manage', 'fulfilment.view.admin', 'fulfilment.retry.admin', 'manifest.view', 'supplier.quality.view.admin', 'supplier.quality.policy.manage', 'supplier.quality.snapshot.manage', 'supplier.quality.issue.manage', 'supplier.tier.manage', 'demand.view.admin', 'demand.manage', 'demand.policy.manage', 'demand.target.manage', 'distribution.view', 'distribution.channel.manage', 'distribution.contract.manage', 'distribution.mapping.manage', 'distribution.inventory-rule.manage', 'distribution.credential.manage', 'distribution.event.view', 'distribution.inspect']);
  ...
  const qualityOwn: Permission[] = ['supplier.quality.view.own']; const demandVendorView: Permission[] = ['demand.view.vendor']; const demandVendorRespond: Permission[] = ['demand.respond.vendor'];
  const roleMap: Record<OrganizationRole, Permission[]> = { OWNER: ['vendor.profile.view', 'vendor.profile.edit', 'document.view', 'document.upload', 'product.view', 'product.edit', 'product.submit', 'rateplan.view', 'rateplan.edit', 'commercial.vendor.view', 'commercial.vendor.edit', 'inventory.view', 'inventory.edit', 'schedule.view', 'schedule.edit', 'resource.view', 'resource.edit', 'booking.view', 'booking.confirm', 'booking.cancel', 'booking.cancel.vendor', 'booking.view.vendor', 'booking.confirm.vendor', 'booking.reject.vendor', 'fulfilment.view.vendor', 'fulfilment.edit.vendor', 'manifest.view', 'manifest.checkin', 'payout.view', 'settlement.view', 'audit.view', ...qualityOwn, ...demandVendorView, ...demandVendorRespond], CATALOGUE: [...catalogue, 'booking.view.vendor', 'fulfilment.view.vendor', ...qualityOwn, ...demandVendorView, ...demandVendorRespond], OPERATIONS: [...operations, 'booking.view.vendor', 'booking.confirm.vendor', 'booking.reject.vendor', ...qualityOwn, ...demandVendorView, ...demandVendorRespond], FINANCE: [...finance, 'booking.view.vendor', 'refund.view', ...qualityOwn], VIEWER: [...viewer, 'booking.view.vendor', ...qualityOwn, ...demandVendorView] };
